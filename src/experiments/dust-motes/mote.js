...
import normalizedRandom from 'koalition-utils/math/normalized-random';
...
export default class Mote {
  ...
  constructor({
    ...
    this._velocity = {
      x: (2 * Math.random() + 4) * 0.01 * this._radius,
      y: (2 * Math.random() + 4) * 0.01 * this._radius,
   ...
   * Check boundary
   */
  boundaryCheck(width, height, threshold = 0.2) {
    const { x, y } = this._pos;
