 ...
 */
let activePointer;
const getPointerEventPosition = event => {
    let x, y;
...
canvas.element.addEventListener('mousemove', pointerListener, {
  passive: true,
});
canvas.element.addEventListener('touchmove', pointerListener, {
  passive: true,
});
...
animator.start(() => {
  ...
  for (var i = 0; i < pointsLength; i++) {
    ...
    color =
      Math.round(LINE_COLOR[0] * decay) +
      ', ' +
      Math.round(LINE_COLOR[1] * decay) +
      ', ' +
      Math.round(LINE_COLOR[2] * decay);
    ...
    if (previousPoint) {
      ...
      for (
        var t = i < CONNECTOR_POINTS ? 0 : i - CONNECTOR_POINTS;
        t < i;
        t++
      ) {
