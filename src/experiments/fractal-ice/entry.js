...
// import roundTo from 'koalition-utils/math/round-to';
// import normalizedRandom from 'koalition-utils/normalized-random';
...
class Comet {
  ...
  _move() {
    ...
    const point = {
      x: convertRange(
        simplex.noise2D(offsetX.value, offsetX.offset),
        -1,
        1,
        0,
        canvas.width
      ...
      y: convertRange(
        simplex.noise2D(offsetY.value, offsetY.offset),
        -1,
        1,
        0,
        canvas.height
    ...
    const speed = lastPoint
      ? Math.abs(point.x - lastPoint.x) + Math.abs(point.y - lastPoint.y)
      : 0;

    if (speed && speed < minSpeed) this._vars.isStopping = true;

    if (age > lifespan || (this._vars.isStopping && speed > lastSpeed)) {
      this._stop();
    }
...
window.comet = new Comet({
  canvas,
  context,
  init: () => context.clearRect(0, 0, canvas.width, canvas.height),
});
