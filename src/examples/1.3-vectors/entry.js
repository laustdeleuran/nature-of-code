...
const canvas = new Canvas({
	...
	clear = () => context.clearRect(-canvas.width/2, -canvas.height/2, canvas.width, canvas.height);
...
function draw(pointerVector) {
	...
	context.rect(-canvas.width / 2, -canvas.height / 2, pointerVector.magnitude, 10);
	context.fillStyle = 'rgba(255, 255, 255, 0.25)';
	context.fill();

	// Draw line 50px towards pointer
	var lineVector = pointerVector.copy().normalize().multiply(50);
	context.beginPath();
	context.moveTo(centerVector.x, centerVector.y);
	context.lineWidth = 3;
...
canvas.element.addEventListener('mousemove', function (event) {
	clear();
	draw(new Vector(event.clientX - canvas.width / 2, event.clientY - canvas.height / 2));
}, { passive: true });
...
canvas.element.addEventListener('touchmove', function (event) {
	...
	for (let i = 0; i < event.touches.length; i++) {
		...
		draw(new Vector(touch.clientX - canvas.width / 2, touch.clientY - canvas.height / 2));
	}
}, { passive: true });
